 ...
 *
 * 5 žingsniai (UAT #42 — perskirstyti AM/teikėjo laukai):
 ...
 *   5. Peržiūra + Pateikti
 *
 * UAT #42 (PA-002/003/004) — teikėjas nebepildo administracinių/finansinių
 * sprendimų: prioriteto, pirkimo stadijos, finansavimo šaltinio laukų ir
 * biudžeto kategorijos. Šiuos nustato AM patvirtindama prašymą (žr.
 * PrasymoDetailPage sprendimo formą). Modelio laukai lieka (nullable) —
...
import type { FinancingRequest, RequestPayload } from '@biip-finansai/shared';
...
// UAT #42 (PA-004): „Biudžetas" žingsnis pašalintas — biudžeto kategorizaciją
// nustato AM patvirtinimo metu. Liko 5 žingsniai.
const STEPS: StepDef[] = [
...
function fromRequest(r: FinancingRequest): FormState {
  return {
    year:
      r.year !== undefined && r.year !== null ? String(r.year) : String(new Date().getFullYear()),
...
function toPayload(state: FormState): RequestPayload {
  ...
  // UAT #42: teikėjas nebepildo priority/procurementStage/funding/biudžeto —
  // tuos laukus nustato AM patvirtinimo metu. Čia jų NEsiunčiam (lieka esamos
  // DB reikšmės / null).
  return {
...
function totalQuarterlyFrom(state: FormState): number {
  return num(state.q1Amount) + num(state.q2Amount) + num(state.q3Amount) + num(state.q4Amount);
}
...
export function RequestWizard({ request, onSaved }: RequestWizardProps): JSX.Element {
  ...
  function validateStep(): string | null {
    ...
    // quarterly step (2) — UAT #42: Biudžetas žingsnis pašalintas, ketvirtinis
    // paskirstymas dabar 3-as žingsnis (index 2).
    if (step === 2) {
    ...
    <div className="mx-auto max-w-5xl p-4 md:p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {request.projectName || 'Be pavadinimo'} · {request.tenantCode}
        </p>
      </div>
          ...
          <CardContent className="space-y-4 p-6">
            {step === 0 && (
              <>
                ...
                {/* UAT #42 (PA-002): Prioritetas + Pirkimo stadija perkelti į
                    AM sprendimo formą — teikėjas jų nebepildo. */}
              </>
            )}
            ...
            {step === 1 && (
              <>
                ...
                {/* UAT #42 (PA-003): Finansavimas iš IT / kitos lėšos / kitų lėšų
                    šaltinis — perkelti į AM sprendimo formą. Teikėjas nurodo tik
                    planuojamas išlaidas; finansavimo šaltinį nustato AM. */}
              </>
            )}

            {/* UAT #42 (PA-004): „Biudžetas" žingsnis pašalintas — biudžeto
                kategorizaciją nustato AM patvirtinimo metu (žr. PrasymoDetailPage). */}

            {step === 2 && (
              <>
                <h2 className="text-lg font-semibold">{STEPS[2]?.label}</h2>
                <p className="text-xs text-muted-foreground">
                  Įveskite planuojamą lėšų panaudojimą pagal ketvirčius. Suma turi atitikti „Iš viso
                  prašoma" = {fmtEur(totalReq)}.
                </p>
                ...
                <div className="rounded-md bg-muted/50 p-3 text-sm">
                  ...
                  <div
                    className={cn(
                      ...
                      Math.abs(quarterlyDiff) > 0.01 ? 'text-destructive' : 'text-muted-foreground',
                    )}
                  >
            ...
            {step === 4 && (
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  Patikrinkite duomenis ir paspauskite „Pateikti". Po pateikimo prašymas keliauja AM
                  tvirtinimui.
                </p>
      ...
      <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-2">
          <Button type="button" variant="outline" onClick={() => navigate('/prasymai')}>
            Atšaukti
          </Button>
          {step > 0 && (
